...
describe('correctAddress', () => {
  ...
  test(' tam adresten ilçe (il var, ilçe yok)', () => {
    const r = correctAddress({ tamAdres: 'Kadıköy Mah. Caferağa Sk. No:5', il: 'İstanbul', ilce: '' }, DATA_DIR);
    expect(r.il).toBe('ISTANBUL');
    expect(r.ilce).toBe('KADIKOY');
    expect(r.corrected).toBe(true);
    expect(r.confidence).toBe('resolved');
  ...
  test(' tam adres ilçe yoksa ilçe boş kalır', () => {
    const r = correctAddress({ tamAdres: 'Bilinmeyen Mah. 1. Cad.', il: 'ANKARA', ilce: '' }, DATA_DIR);
    expect(r.il).toBe('ANKARA');
    expect(r.ilce).toBe('');
  });

  test(' tam adreste Çankaya geçiyor → ilçe dolar', () => {
    const r = correctAddress({ tamAdres: 'Çankaya, Kızılay', il: 'Ankara', ilce: '' }, DATA_DIR);
    expect(r.il).toBe('ANKARA');
    expect(r.ilce).toBe('CANKAYA');
    expect(r.corrected).toBe(true);
  });
  ...
  test(' tamAdres ve postaKodu aynen döner', () => {
    const r = correctAddress({ tamAdres: 'X Sokak', il: 'ADANA', postaKodu: '01000' }, DATA_DIR);
  ...
  test('correctAddressBatch: toplu', () => {
    const list = correctAddressBatch([{ il: 'ankara' }, { ilce: 'SEYHAN' }, { il: 'ADANA', ilce: 'CEYHAN' }], DATA_DIR);
  ...
  test(' düzeltme olsa bile postaKodu ve tamAdres korunur', () => {
    const r = correctAddress({ tamAdres: 'Test Sokak No:1', il: 'anakra', ilce: 'cankaya', postaKodu: '06420' }, DATA_DIR);
  ...
  test(' tam adreste birden fazla ilçe adı - uzun eşleşir', () => {
    const r = correctAddress({ tamAdres: 'Çukurova ve Seyhan arası', il: 'ADANA', ilce: '' }, DATA_DIR);
    expect(r.il).toBe('ADANA');
  ...
  // --- Çeşitli edge case ve kombinasyonlar ---

  test('il alanında Turkey (İngilizce) → ilçe il sayılır, ilçe tam adresten', () => {
    const r = correctAddress({ il: 'Turkey', ilce: 'Istanbul', tamAdres: 'Üsküdar Çarşı' }, DATA_DIR);
    expect(r.il).toBe('ISTANBUL');
    expect(r.ilce).toBe('USKUDAR');
    expect(r.corrected).toBe(true);
  });

  test('il alanında ilçe (Seyhan) → Adana + Seyhan', () => {
    const r = correctAddress({ il: 'Seyhan', ilce: 'Yüreğir' }, DATA_DIR);
    expect(r.il).toBe('ADANA');
    expect(r.ilce).toBe('SEYHAN');
    expect(r.corrected).toBe(true);
  });

  test('il alanında ilçe (Kadıköy), ilçe alanı boş → İstanbul + Kadıköy', () => {
    const r = correctAddress({ il: 'Kadıköy', ilce: '' }, DATA_DIR);
    expect(r.il).toBe('ISTANBUL');
    expect(r.ilce).toBe('KADIKOY');
    expect(r.corrected).toBe(true);
  });

  test('tamAdres uzun, ilçe kısa - ilçe/İl tamAdres’te aranır', () => {
    const longTam = 'A Mah. B Cad. No:1 Çok Uzun Bir Sokak Adı Kartal İlçesi İstanbul İli Kartal/ISTANBUL ek bilgi';
    const r = correctAddress({ il: '', ilce: 'X', tamAdres: longTam }, DATA_DIR);
    expect(r.il).toBe('ISTANBUL');
    expect(r.ilce).toBe('KARTAL');
    expect(r.corrected).toBe(true);
  });

  test('metinde birden fazla ilçe/İl - ilk geçerli kullanılır', () => {
    const text = 'Yanlış Bilmem/XYZ Mah. Uzun Açıklama Etimesgut/ANKARA Doğru İlçe İl Formatı';
    const r = correctAddress({ il: '', ilce: text, tamAdres: text }, DATA_DIR);
    expect(r.il).toBe('ANKARA');
    expect(r.ilce).toBe('ETIMESGUT');
  });

  test('sadece tamAdres (il/ilçe boş) - çözülemez, boş döner', () => {
    const r = correctAddress({ tamAdres: 'Kadıköy Mah. No:5', il: '', ilce: '' }, DATA_DIR);
    expect(r.il).toBe('');
    expect(r.ilce).toBe('');
    expect(r.corrected).toBe(false);
  });

  test('ülke + ilçe ama tamAdres boş - il ilçeden, ilçe boş kalır', () => {
    const r = correctAddress({ il: 'Türkiye', ilce: 'Ankara', tamAdres: '' }, DATA_DIR);
    expect(r.il).toBe('ANKARA');
    expect(r.ilce).toBe('');
    expect(r.corrected).toBe(true);
  });

  test('tanınmayan il + tanınan ilçe → ilçeden il çözülür', () => {
    const r = correctAddress({ il: 'XyzAbc', ilce: 'Kadıköy' }, DATA_DIR);
    expect(r.il).toBe('ISTANBUL');
    expect(r.ilce).toBe('KADIKOY');
    expect(r.corrected).toBe(true);
  });

  test('correctAddressBatch: boş ve bilinmeyen karışık', () => {
    const list = correctAddressBatch(
      [
        { il: '', ilce: '' },
        { il: 'XyzAbc123', ilce: 'QweRty456' },
        { il: 'adana', ilce: 'seyhan' },
      ],
      DATA_DIR,
    );
    expect(list.length).toBe(3);
    expect(list[0].il).toBe('');
    expect(list[0].ilce).toBe('');
    expect(list[1].il).toBe('XyzAbc123');
    expect(list[1].ilce).toBe('QweRty456');
    expect(list[2].il).toBe('ADANA');
    expect(list[2].ilce).toBe('SEYHAN');
  });

  test('correctAddressBatch: tamAdres ile tam adresten ilçe', () => {
    const list = correctAddressBatch(
      [
        { tamAdres: 'Çankaya Kızılay', il: 'Ankara', ilce: '' },
        { tamAdres: 'Kadıköy Moda', il: 'İstanbul', ilce: '' },
      ],
      DATA_DIR,
    );
    expect(list[0].ilce).toBe('CANKAYA');
  ...
  test('kısa il/ilçe - tek kelime typo', () => {
    const r = correctAddress({ il: 'Izmir', ilce: 'Karsiyaka' }, DATA_DIR);
    expect(r.il).toBe('IZMIR');
    expect(r.ilce).toBe('KARSIYAKA');
  });

  test('postaKodu undefined - aynen korunur (boş string)', () => {
    const r = correctAddress({ il: 'ANKARA', ilce: 'CANKAYA' }, DATA_DIR);
    expect(r).toHaveProperty('postaKodu', '');
  });

  test('çıktı yapısı - CorrectedAddress alanları her zaman var', () => {
    const r = correctAddress({ il: 'adana', ilce: 'ceyhan' }, DATA_DIR);
    expect(r).toHaveProperty('il');
    expect(r).toHaveProperty('ilce');
    expect(r).toHaveProperty('tamAdres');
    expect(r).toHaveProperty('postaKodu');
    expect(r).toHaveProperty('corrected');
    expect(r).toHaveProperty('confidence');
    expect(typeof r.corrected).toBe('boolean');
  });

  test('ilce 50 karakterden kısa - sadece ilçe gibi işlenir (il yoksa)', () => {
    const r = correctAddress({ il: '', ilce: 'Kadıköy', tamAdres: 'Kısa adres' }, DATA_DIR);
    expect(r.il).toBe('ISTANBUL');
    expect(r.ilce).toBe('KADIKOY');
  });

  test('il doğru ilçe typo - confidence fuzzy', () => {
    const r = correctAddress({ il: 'ANKARA', ilce: 'Cankya' }, DATA_DIR);
    expect(r.confidence).toBe('fuzzy');
  });

  test('sadece ilçe (Çankaya) - confidence resolved', () => {
    const r = correctAddress({ ilce: 'Çankaya' }, DATA_DIR);
    expect(r.confidence).toBe('resolved');
  });
});
